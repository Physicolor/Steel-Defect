...
class RecordService:
    ...
    def delete_capture(self, filename: str):
        """删除单个检测记录文件"""
    ...
    def batch_delete(self, filenames: list):
        """批量删除检测记录（包括图片和关联的info.json）"""
        import re
        deleted = []
        # 收集所有要删除的文件（包括关联的 info.json）
        all_files = set(filenames)
        for fn in filenames:
            # 从文件名提取 batch_id，查找关联的 info.json
            m = re.match(
                r'(image_batch|camera_batch|ip_batch|batch)_(.+?)_(original|annotated|heatmap|crop_\d+_.+)\.\w+$',
                fn
            )
            if m:
                prefix = m.group(1)
                ts_part = m.group(2)
                batch_id = f"{prefix}_{ts_part}"
                info_file = f"{batch_id}_info.json"
                all_files.add(info_file)

        for fn in all_files:
            if self.delete_capture(fn):
                deleted.append(fn)
        return deleted
    ...
    def get_recent_stats(self):
        """获取近期统计（最近7天有数据的日期） - 返回 {daily_stats: {YYYY-MM-DD: {total, defect}}}"""
        ...
        for rec in records:
            ...
            if ts and len(ts) >= 8:
                ...
                if date_str not in all_daily_stats:
                    all_daily_stats[date_str] = {'total': 0, 'defect': 0}
                all_daily_stats[date_str]['total'] += 1
                defects = rec.get('defects', [])
                if defects and len(defects) > 0:
                    all_daily_stats[date_str]['defect'] += 1

        # 按日期倒序排序，取最近7天有数据的日期
        sorted_dates = sorted(all_daily_stats.keys(), reverse=True)[:7]

        # 构建最终结果（按日期正序）
        daily_stats = {}
        for date_str in sorted(sorted_dates):
            daily_stats[date_str] = all_daily_stats[date_str]
