...
class SparkLiteService:
    ...
    def _create_url(self) -> str:
        ...
        if not self.app_id or not self.api_key or not self.api_secret:
            raise ValueError("讯飞星火Lite API认证信息未配置，请设置环境变量: SPARK_LITE_APP_ID, SPARK_LITE_API_KEY, SPARK_LITE_API_SECRET")
    ...
    def _build_default_prompt(self, defect_stats: Dict, daily_stats: Dict,
        ...
        if damage_ratio:
            # 兼容两种格式:
            # 1. {total, with_defects, without_defects, damage_ratio} (前端发送)
            # 2. {defect_type: {'avg_area_ratio': ..., 'total_count': ...}} (旧格式)
            if 'total' in damage_ratio:
                total = damage_ratio.get('total', 0)
                with_defects = damage_ratio.get('with_defects', 0)
                without_defects = damage_ratio.get('without_defects', 0)
                ratio = damage_ratio.get('damage_ratio', 0)
                damage_desc += f"- 总检测记录: {total}条\n"
                damage_desc += f"- 有缺陷记录: {with_defects}条\n"
                damage_desc += f"- 无缺陷记录: {without_defects}条\n"
                damage_desc += f"- 缺陷率: {ratio}%\n"
            else:
                for defect_type, stats in sorted(damage_ratio.items(), key=lambda x: x[1].get('avg_area_ratio', 0), reverse=True):
